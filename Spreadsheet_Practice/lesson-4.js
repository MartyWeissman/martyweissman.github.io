...
var Lesson4 = (function () {
  ...
  var exercises = [
    {
      ...
      instruction: `<p>In this lesson, we'll explore <strong>absolute cell references</strong> while modeling a fascinating mathematical concept: the <strong>discrete logistic model</strong>.</p>
    ...
    },
    {
      ...
      validate: function () {
        ...
        const timeFormulaCorrect =
          timeUpdate.type === "formula" &&
          timeUpdate.formula
            .toLowerCase()
            .replace(/\s+/g, "")
            .includes("a2+c2") &&
        ...
        const pFormulaCorrect =
          pUpdate.type === "formula" &&
        ...
        const dpFormulaCorrect =
          dpFormula.type === "formula" &&
          ...
          (dpFormula.formula
            ...
            dpFormula.formula
              .toLowerCase()
              .replace(/\s+/g, "")
              .includes("e2*b2-e2*b2*b2") ||
            dpFormula.formula
              .toLowerCase()
              .replace(/\s+/g, "")
              .includes("e2*b2-e2*b2^2")) &&
    ...
    },
    {
      ...
      validate: function () {
        ...
        const timeCorrect = time3.type === "formula" && time3.result === 1;

        // Check P value is correct
        const pCorrect =
          p3.type === "formula" && Math.abs(p3.result - 0.298) < 0.001;

        // Check Δt value is correct
        const dtCorrect = dt3.type === "number" && dt3.result === 1;
    ...
    },
    {
      ...
      validate: function () {
        ...
        // Check time value is correct
        const timeCorrect = time8.type === "formula" && time8.result === 6;

        // Check Δt value is correct
        const dtCorrect = dt8.result === 1;

        // Check that D2 uses absolute reference
        const absoluteRefCorrect =
          dp2.type === "formula" && dp2.formula.includes("$E$2");
  ...
  function showLessonIntroduction() {
    var introduction = `
        ...
        <p>Welcome to Lesson 4! Advanced spreadsheet techniques:</p>
        <ul>
            <li><strong>Absolute References:</strong> Using $ symbols effectively</li>
            <li><strong>Logistic Models:</strong> Population growth with limits</li>
            <li><strong>Complex Formulas:</strong> Multi-variable equations</li>
        </ul>
