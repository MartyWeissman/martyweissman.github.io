...
// Use shared global student name across all lessons
if (typeof window.studentName === "undefined") {
  window.studentName = "student";
}

// Define the lesson exercises
window.lesson1Exercises = [
  {
    ...
    validate: function () {
      ...
      if (nameCell.type === "text" && nameCell.value.trim() !== "") {
        window.studentName = nameCell.value.trim();
      }
  ...
  },
  {
    ...
    getSuccessMessage: function () {
      ...
      if (typeof window.makeVerification === "function") {
        verificationCode = window.makeVerification(window.studentName, 1);
      }
...
// Make the initialization function available globally with unique name
window.initLesson1 = initLesson;
...
function handlePracticeButtonClick() {
  ...
  if (buttonState === "Start Exercises") {
    ...
    const exercise = window.lesson1Exercises[currentExerciseIndex];
