...
const lesson2Exercises = [
    {
        ...
        instruction:
            `<p>In this lesson, we'll learn about copying formulas, how relative references work, and some useful keyboard shortcuts.</p>
        ...
        getErrorMessage: function() {
            ...
            if (popHeader.type !== 'text' || !popHeader.value.toLowerCase().includes('population')) {
    ...
    },
    {
        ...
        validate: function() {
            ...
            const time4Correct = time4.type === 'formula' &&
                                 Math.abs(time4.result - expectedTimes[0]) < tolerance;

            const pop4Correct = pop4.type === 'formula' &&
                               Math.abs(pop4.result - expectedPops[0]) < tolerance;

            const time5Correct = time5.type === 'formula' &&
                                Math.abs(time5.result - expectedTimes[1]) < tolerance;

            const pop5Correct = pop5.type === 'formula' &&
                               Math.abs(pop5.result - expectedPops[1]) < tolerance;

            const time6Correct = time6.type === 'formula' &&
                                Math.abs(time6.result - expectedTimes[2]) < tolerance;

            const pop6Correct = pop6.type === 'formula' &&
                               Math.abs(pop6.result - expectedPops[2]) < tolerance;

            return time4Correct && pop4Correct &&
                   time5Correct && pop5Correct &&
                   time6Correct && pop6Correct;
        },
        getErrorMessage: function() {
    ...
    },
    {
        ...
        validate: function() {
            ...
            return initialCorrect && pop3Correct && pop4Correct && pop5Correct && pop6Correct;
        },
        getErrorMessage: function() {
            const initialPop = getCellData('B2');

            if (initialPop.type !== 'number' || Math.abs(initialPop.result - 1000) >= 0.1) {
...
function handlePracticeButtonClick() {
    ...
    if (buttonState === 'Start Exercises') {
        ...
        const currentExercise = lesson2Exercises[currentExerciseIndex];
        ...
        if (currentExercise.validate()) {
            ...
            if (currentExerciseIndex < lesson2Exercises.length) {
        ...
        // Load the next lesson (Lesson 3)
        const lessonButtons = document.querySelectorAll('.lesson-button');
        if (lessonButtons.length >= 3) {
            lessonButtons[2].click();  // Click Lesson 3 button
        }
    }
}
...
function showExercise(index) {
    if (index < 0 || index >= lesson2Exercises.length) {
        console.error(`Exercise index ${index} is out of range`);
        return;
    }

    const exercise = lesson2Exercises[index];
