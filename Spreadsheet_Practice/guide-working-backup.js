  ...
  if (practiceButton) {
    ...
    newButton.addEventListener("click", function () {
      if (this.textContent === "Start Practice") {
        ...
        const lesson1Button = document.querySelector(
          '.lesson-button[data-lesson="1"]',
        );
        if (lesson1Button) {
...
function loadLesson(lessonNumber) {
  ...
  const lessonButton = document.querySelector(
    `.lesson-button[data-lesson="${lessonNumber}"]`,
  );
  if (lessonButton) {
...
function checkURLForLesson() {
  // Cross-browser URL parameter parsing (works in all browsers including IE9+)
  const lessonParam = (function () {
    const match = window.location.search.match(/[?&]lesson=([^&]*)/);
